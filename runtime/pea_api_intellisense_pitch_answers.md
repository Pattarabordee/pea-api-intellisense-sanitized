...
- API endpoint รับ request แบบ authenticated ได้
- Request ที่ไม่มี key ถูกปฏิเสธด้วย `401 Unauthorized`
- ระบบเก็บ audit evidence แบบ redacted ได้
- Duplicate request ไม่ควร reprocess
- มี OpenAPI, Postman, handoff note, runbook, deck และ demo support

สถานะที่อ้างอิงได้:

- ใช้ authenticated GET-only guard เท่านั้น
- รายงานปัจจุบันอยู่ใน private path และใช้ hash reference แทน identifier ดิบ
- หน้า public เป็น `DEMO DATA` ไม่ใช่หลักฐาน operational
- count หรือ identifier จาก snapshot/local-tunnel เก่าเป็นข้อมูล superseded ห้ามใช้ในบทความหรือการตัดสินใจ

ข้อจำกัดที่ต้องพูดให้ชัด:

- ยังไม่ใช่ production live
- ห้ามใช้ local tunnel หรือ snapshot เก่าเป็นหลักฐาน production
