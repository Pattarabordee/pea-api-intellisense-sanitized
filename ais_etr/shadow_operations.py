...
SIMPLE_MAPPING_RESPONSE_COLUMNS = (
    "site_ref",
    "location_id",
    "sitecode",
    "ais_answer",
...
OWNER_RESPONSE_PREFLIGHT_COLUMNS = (
    "file_role",
    "path",
    "file_status",
    "schema_status",
    ...
    "blank_rows",
    "missing_columns",
    "private_identifier_cells",
    "sensitive_field_cells",
    "recommended_action",
)
...
def build_owner_response_preflight(
    ...
    *,
    ...
    mapping_profile = _owner_response_file_profile("mapping_response", mapping_response_csv)
    webex_profile = _owner_response_file_profile("webex_truth_response", webex_response_csv)
    rows = [mapping_profile, webex_profile]
    _write_csv(output_csv, OWNER_RESPONSE_PREFLIGHT_COLUMNS, rows)
    readme_path = Path(readme_output) if readme_output else _default_owner_response_readme(mapping_response_csv, webex_response_csv)
    readme_path.parent.mkdir(parents=True, exist_ok=True)
    readme_path.write_text(_render_owner_response_folder_readme(mapping_response_csv, webex_response_csv), encoding="utf-8-sig")
    status_counts = Counter(row["file_status"] for row in rows)
    schema_counts = Counter(row["schema_status"] for row in rows)
    summary = {
        "rows": len(rows),
        "ready_files": sum(1 for row in rows if row["file_status"] == "has_owner_response_rows" and row["schema_status"] == "ok"),
        "missing_files": sum(1 for row in rows if row["file_status"] == "missing"),
        "sensitive_field_cells": sum(_to_int(row["sensitive_field_cells"]) or 0 for row in rows),
        "private_identifier_cells": sum(_to_int(row["private_identifier_cells"]) or 0 for row in rows),
        "file_status_counts": dict(status_counts.most_common()),
        "schema_status_counts": dict(schema_counts.most_common()),
        "output_csv": str(output_csv),
        "markdown_output": str(markdown_output),
        "readme_output": str(readme_path),
    }
    Path(markdown_output).parent.mkdir(parents=True, exist_ok=True)
    Path(markdown_output).write_text(_render_owner_response_preflight(summary, rows), encoding="utf-8-sig")
    return summary
...
def validate_owner_response_files(
    ...
    *,
    ...
    preflight_markdown_output: str | Path | None = None,
    response_readme_output: str | Path | None = None,
) -> dict[str, Any]:
    preflight_summary = build_owner_response_preflight(
        mapping_response_csv,
        webex_response_csv,
        preflight_output or Path(output_csv).with_name("owner_response_preflight.csv"),
        preflight_markdown_output or Path(markdown_output).with_name("owner_response_preflight.md"),
        readme_output=response_readme_output,
    )
    rows = []
...
def _owner_response_file_profile(file_role: str, path: str | Path) -> dict[str, str]:
    source = Path(path)
    if not source.exists():
        return {
            ...
            "blank_rows": "0",
            "missing_columns": "",
            "private_identifier_cells": "0",
            "sensitive_field_cells": "0",
            "recommended_action": "Place the completed owner response CSV at this path, or keep waiting_for_owner_response.",
        }
    fieldnames, rows = _read_csv_with_header(source)
    schema_status, missing_columns = _owner_response_schema_status(file_role, fieldnames)
    filled_rows = sum(1 for row in rows if _owner_response_row_has_response(file_role, row))
    blank_rows = max(len(rows) - filled_rows, 0)
    private_identifier_cells = _count_private_identifier_cells(rows)
    sensitive_field_cells = _count_sensitive_field_cells(fieldnames, rows)
    if not fieldnames:
        file_status = "empty_file"
    elif filled_rows:
        file_status = "has_owner_response_rows"
    elif rows:
        file_status = "template_only"
    else:
        file_status = "empty_response_file"
    return {
        "file_role": file_role,
        "path": str(source),
        ...
        "blank_rows": str(blank_rows),
        "missing_columns": ", ".join(missing_columns),
        "private_identifier_cells": str(private_identifier_cells),
        "sensitive_field_cells": str(sensitive_field_cells),
        "recommended_action": _owner_response_preflight_action(
            file_status,
            schema_status,
            filled_rows,
            private_identifier_cells,
...
def _read_csv_with_header(path: Path) -> tuple[tuple[str, ...], list[dict[str, str]]]:
...
def _owner_response_schema_status(file_role: str, fieldnames: tuple[str, ...]) -> tuple[str, tuple[str, ...]]:
    if not fieldnames:
        expected = SIMPLE_MAPPING_RESPONSE_COLUMNS if file_role == "mapping_response" else WEBEX_MIN_RESPONSE_COLUMNS
        return "missing_columns", expected
    variants = (
        (OWNER_RESPONSE_TEMPLATE_COLUMNS, SIMPLE_MAPPING_RESPONSE_COLUMNS)
        if file_role == "mapping_response"
        else (OWNER_RESPONSE_TEMPLATE_COLUMNS, WEBEX_MIN_RESPONSE_COLUMNS)
    )
    fieldset = set(fieldnames)
    missing_by_variant = [tuple(column for column in variant if column not in fieldset) for variant in variants]
    best_missing = min(missing_by_variant, key=len)
    return ("ok", ()) if not best_missing else ("missing_columns", best_missing)


def _owner_response_row_has_response(file_role: str, row: dict[str, str]) -> bool:
    if file_role == "mapping_response":
        ...
        return _has_owner_mapping_response(row)
    fields = (
        "owner_decision",
        "mapped_site_id",
        "mapped_site_code",
...
def _count_private_identifier_cells(rows: list[dict[str, str]]) -> int:
    private_fields = {
        "confirmed_peano_or_meter_id",
        "mapped_site_id",
        "mapped_site_code",
    ...
    for row in rows:
        for field, value in row.items():
            if str(field or "").strip().lower() in private_fields and str(value or "").strip():
                count += 1
    return count


def _count_sensitive_field_cells(fieldnames: tuple[str, ...], rows: list[dict[str, str]]) -> int:
    sensitive_tokens = ("token", "secret", "room_id", "roomid", "raw_webex", "webex_text", "access_token", "refresh_token")
    sensitive_fields = {field for field in fieldnames if any(token in str(field or "").lower() for token in sensitive_tokens)}
    count = 0
    for row in rows:
        for field, value in row.items():
            text = str(value or "")
            field_text = str(field or "").lower()
            if field in sensitive_fields and text.strip():
                count += 1
            elif any(token in field_text for token in sensitive_tokens) and text.strip():
                count += 1
            elif _looks_like_secret_value(text):
                count += 1
    return count
...
def _looks_like_secret_value(value: str) -> bool:
    ...
    if not text:
        return False
    if any(marker in lowered for marker in ("bearer ", "refresh_token", "access_token", "webexapis.com/v1/messages")):
        return True
    if text.startswith(("sk-", "sk_proj_", "Bearer ")):
        return True
    return len(text) >= 80 and any(ch.isdigit() for ch in text) and any(ch.isalpha() for ch in text) and (" " not in text)


def _owner_response_preflight_action(
    file_status: str,
    schema_status: str,
    filled_rows: int,
    private_identifier_cells: int,
    sensitive_field_cells: int,
) -> str:
    if file_status == "missing":
        return "Place completed owner response CSV under runtime/owner_responses before expecting ready rows."
    if schema_status != "ok":
        return "Use the generated owner response template columns, then rerun owner-response-validate."
    if sensitive_field_cells:
        return "Remove tokens, room ids, raw WebEx text, and secret-bearing fields before sharing reports."
    if not filled_rows:
        return "Template is present but no owner answers are filled yet."
    if private_identifier_cells:
        return "Private identifiers are counted only; keep this file private and use validation/intake reports for sharing."
    return "Run validation, intake, dry-run impact, then daily-shadow-refresh."


def _default_owner_response_readme(mapping_response_csv: str | Path, webex_response_csv: str | Path) -> Path:
    mapping_parent = Path(mapping_response_csv).parent
    webex_parent = Path(webex_response_csv).parent
    return (mapping_parent if mapping_parent == webex_parent else mapping_parent) / "README.md"
...
def _render_owner_message_drafts(summary: dict[str, Any], sources: dict[str, Path]) -> str:
    lines = [
        ...
        "",
        "ใช้ร่างข้อความนี้ส่งต่อให้เจ้าของข้อมูล โดยแนบไฟล์ตาม path ที่ระบุและห้ามส่ง credentials, WebEx room identifier, raw WebEx text หรือข้อมูลลูกค้าที่ไม่จำเป็น",
        "",
...
def _render_owner_response_preflight(summary: dict[str, Any], rows: list[dict[str, str]]) -> str:
    lines = [
        "# Owner Response Preflight",
        "",
        "This checks returned owner response files before validation. It does not apply mapping repair, import truth, send notifications, or promote a model.",
        "",
        f"- Files checked: {summary['rows']}",
        f"- Ready files: {summary['ready_files']}",
        f"- Missing files: {summary['missing_files']}",
        f"- Private identifier cells counted: {summary['private_identifier_cells']}",
        f"- Sensitive field cells counted: {summary['sensitive_field_cells']}",
        f"- Folder README: `{summary['readme_output']}`",
        "",
        "## File Status Counts",
        "",
        *_bullet_counts(summary["file_status_counts"]),
        "",
        "## Schema Status Counts",
        "",
        *_bullet_counts(summary["schema_status_counts"]),
        "",
        "## Files",
        "",
        *_table(rows, OWNER_RESPONSE_PREFLIGHT_COLUMNS),
        "",
        "## Next Command Order",
        "",
        "1. `python -m ais_etr owner-response-validate`",
        "2. `python -m ais_etr owner-response-intake`",
        "3. `python -m ais_etr owner-response-dry-run-impact`",
        "4. `python -m ais_etr daily-shadow-refresh`",
        "",
        "## Guardrail",
        "",
        "- Private identifiers may exist in the private response CSVs, but this report only counts them.",
        "- Do not put tokens, room ids, raw WebEx text, customer names, or full meter lists in shareable reports.",
        "- AIS outage/restore is the only customer-facing truth lane.",
...
def _render_owner_response_folder_readme(mapping_response_csv: str | Path, webex_response_csv: str | Path) -> str:
    lines = [
        "# Owner Responses",
        "",
        "Place completed owner response CSVs in this folder. These files are private operator inputs, not public reports.",
        "",
        "## Expected Files",
        "",
        f"- Mapping repair response: `{mapping_response_csv}`",
        f"- WebEx AIS truth response: `{webex_response_csv}`",
        "",
        "## Required Flow",
        "",
        "1. Fill only owner response columns.",
        "2. Run `python -m ais_etr owner-response-validate`.",
        "3. Run `python -m ais_etr owner-response-intake`.",
        "4. Run `python -m ais_etr owner-response-dry-run-impact`.",
        "5. Run `python -m ais_etr daily-shadow-refresh` after review.",
        "",
        "## Guardrails",
        "",
        "- Do not paste API keys, tokens, room ids, raw WebEx text, customer names, or full meter lists here.",
        "- Mapping answers can include private meter/site identifiers only in the private response CSV.",
        "- AIS truth must use AIS outage/restore timestamps, not PEA/SFSD/ReportPO restore labels.",
        "- Short interruptions `<=5` minutes are review-only for sustained ETR gate.",
...
def _render_owner_response_validation(summary: dict[str, Any], rows: list[dict[str, str]]) -> str:
    lines = [
        ...
        f"- Preflight ready files: {summary['preflight']['ready_files']}",
        f"- Preflight missing files: {summary['preflight']['missing_files']}",
        f"- Preflight sensitive field cells: {summary['preflight']['sensitive_field_cells']}",
        "",
        "## Status Counts",
        "",
        *_bullet_counts(summary["status_counts"]),
...
def _render_executive_pitch_pack(summary: dict[str, Any], dry_run_rows: list[dict[str, str]]) -> str:
    lines = [
        ...
        *_table(dry_run_rows, OWNER_DRY_RUN_IMPACT_COLUMNS),
        ...
        "- No production AIS send.",
        "- No overwrite of `runtime/model_quantiles.json`.",
        "- No customer identifier list or raw WebEx text is required for this executive pack.",
        "",
    ]
    return "\n".join(lines)
...
def _render_current_capability_development_plan(summary: dict[str, Any], rows: list[dict[str, str]]) -> str:
    ...
    lines = [
        ...
        *_table(acceptance_rows, CURRENT_CAPABILITY_PLAN_COLUMNS),
        ...
        "- Public reports must avoid credentials, WebEx room identifiers, raw WebEx message bodies, customer meter lists, and unnecessary customer identity fields.",
        "",
    ]
