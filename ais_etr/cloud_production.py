...
def _render_production_gate_packet(summary: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Reports must not include API key, DB URL, token, room ID, raw WebEx text, full meter/PEANO, or customer identity.",
            "",
            "## Outputs",
            "",
        ]
...
def _render_production_approval_evidence_pack(summary: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Do not paste API keys, DB URLs, tokens, room IDs, raw WebEx text, full meter/PEANO, or customer identity into any shared channel.",
...
def _render_mvp_daily_qa_report(summary: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Do not expose API key, DB URL, token, room ID, raw WebEx text, full meter/PEANO, or customer identity.",
...
def _render_mvp_demo_recording_pack(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- Do not show Render env, API key, DB URL, raw WebEx, full meter/PEANO, or customer identity.",
...
def _render_worker_report(summary: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Full meter, PEANO lists, customer identity, room IDs, tokens, and raw WebEx text are not written.",
        ]
    )
    return "\n".join(lines) + "\n"
