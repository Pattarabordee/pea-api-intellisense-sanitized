...
def _normalize_history_message(message: dict[str, Any]) -> dict[str, Any]:
    ...
    if normalized.get("message_id") and not normalized.get("id"):
        normalized["id"] = normalized["message_id"]
    if normalized.get("text_sanitized") and not normalized.get("text"):
        normalized["text"] = normalized["text_sanitized"]
    if normalized.get("chat_id_hash") and not normalized.get("roomId"):
        normalized["roomId"] = normalized["chat_id_hash"]
