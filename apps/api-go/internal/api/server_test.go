...
func TestPostAcceptsValidRequestAndKeepsProductionBlocked(t *testing.T) {
	...
	body := `{"request_id":"AIS-HTTP-1","meter_no":"METER-1234","timestamp":"2026-06-19T17:04:00+07:00"}`
...
func TestDuplicateRequestIsIdempotent(t *testing.T) {
	store := newFakeStore()
	handler := NewServer(ServerConfig{APIKey: "pilot-key"}, store)
	body := `{"request_id":"AIS-DUP","meter_no":"METER-1234","timestamp":"2026-06-19T17:04:00+07:00"}`
	for idx := 0; idx < 2; idx++ {
		req := httptest.NewRequest(http.MethodPost, inboundPath, bytes.NewBufferString(body))
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("X-API-Key", "pilot-key")
...
func TestStatusLookupWorks(t *testing.T) {
	store := newFakeStore()
	handler := NewServer(ServerConfig{APIKey: "pilot-key"}, store)
	body := `{"request_id":"AIS-LOOKUP","meter_no":"METER-1234","timestamp":"2026-06-19T17:04:00+07:00"}`
...
func TestBadTimestampReturns400(t *testing.T) {
	handler := NewServer(ServerConfig{APIKey: "pilot-key"}, newFakeStore())
	body := `{"request_id":"AIS-BAD","meter_no":"METER-1234","timestamp":"not-a-date"}`
	req := httptest.NewRequest(http.MethodPost, inboundPath, bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-API-Key", "pilot-key")
	res := httptest.NewRecorder()
...
func TestLongOptionalFieldReturns400(t *testing.T) {
	handler := NewServer(ServerConfig{APIKey: "pilot-key"}, newFakeStore())
	body := `{"request_id":"AIS-LONG","meter_no":"METER-1234","timestamp":"2026-06-19T17:04:00+07:00","province":"` + strings.Repeat("A", 121) + `"}`
	req := httptest.NewRequest(http.MethodPost, inboundPath, bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-API-Key", "pilot-key")
	res := httptest.NewRecorder()
...
func TestMetricsEndpointIsAuthOnlyAndReportsShadowGuardrails(t *testing.T) {
	store := newFakeStore()
	handler := NewServer(ServerConfig{APIKey: "pilot-key"}, store)
	body := `{"request_id":"AIS-METRICS","meter_no":"METER-1234","timestamp":"2026-06-19T17:04:00+07:00"}`
	for idx := 0; idx < 2; idx++ {
		req := httptest.NewRequest(http.MethodPost, inboundPath, bytes.NewBufferString(body))
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("X-API-Key", "pilot-key")
...
func TestProductionSendModeNeverEnablesRealSendWithoutGates(t *testing.T) {
	...
	body := `{"request_id":"AIS-GATE-BLOCK","meter_no":"METER-1234","timestamp":"2026-06-19T17:04:00+07:00"}`
	req := httptest.NewRequest(http.MethodPost, inboundPath, bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-API-Key", "pilot-key")
	res := httptest.NewRecorder()
