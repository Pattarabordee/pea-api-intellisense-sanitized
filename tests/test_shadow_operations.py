...
from ais_etr.shadow_operations import (
    ...
    build_owner_response_preflight,
...
class ShadowOperationsTests(unittest.TestCase):
    ...
    def test_ais_daily_file_qa_splits_usable_review_and_reject_rows(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_csv(
                ...
                columns,
                [
                    {"site_id": "site-1", "peano": "meter-1", "outage_start_time": "2026-06-19T08:00:00", "power_restore_time": "2026-06-19T08:30:00", "actual_restoration_minutes": "30", "truth_quality": "OK"},
            ...
            _write_csv(review, columns, [{"site_id": "site-3", "peano": "meter-3", "outage_start_time": "2026-06-19T08:00:00", "power_restore_time": "2026-06-19T08:03:00", "actual_restoration_minutes": "3", "truth_quality": "REVIEW_SHORT"}])
    ...
    def test_owner_response_templates_and_validator_keep_short_truth_out_of_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((root / "owner_response_preflight.csv").exists())
            self.assertTrue((root / "README.md").exists())

    def test_owner_response_preflight_counts_private_values_without_leaking_them(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            secret = "sk-proj-" + "A" * 90
            _write_csv(
                ...
                [
                    {
                        ...
                        "confirmed_peano_or_meter_id": "METER-PRIVATE-1234567890",
                        "reviewed_by": "owner",
                        "reviewed_at": "2026-06-19",
                        "access_token": secret,
                    }
            ...
            result = build_owner_response_preflight(
                root / "mapping_response.csv",
                root / "missing_webex_response.csv",
                root / "preflight.csv",
                root / "preflight.md",
                readme_output=root / "owner_responses" / "README.md",
            )
            rows = {row["file_role"]: row for row in _read_csv(root / "preflight.csv")}
            text = (root / "preflight.md").read_text(encoding="utf-8-sig")
            readme = (root / "owner_responses" / "README.md").read_text(encoding="utf-8-sig")

            self.assertEqual(result["missing_files"], 1)
            self.assertEqual(rows["mapping_response"]["file_status"], "has_owner_response_rows")
            self.assertEqual(rows["mapping_response"]["schema_status"], "ok")
            self.assertEqual(rows["mapping_response"]["private_identifier_cells"], "1")
            self.assertEqual(rows["mapping_response"]["sensitive_field_cells"], "1")
            self.assertEqual(rows["webex_truth_response"]["file_status"], "missing")
            self.assertNotIn(secret, text)
            self.assertNotIn("METER-PRIVATE-1234567890", text)
            self.assertIn("Production AIS send remains blocked", readme)
