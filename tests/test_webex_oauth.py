...
class WebexOAuthTests(unittest.TestCase):
    ...
    def test_oauth_polling_omits_mentioned_people_and_bot_polling_keeps_it(self):
        ...
        with patch("urllib.request.urlopen", side_effect=fake_urlopen):
            WebexClient(token_provider=lambda: "oauth-token", room_id="<REDACTED_ROOM_ID>", require_mention=False).list_messages(
                before_message="message-123"
            )
            WebexClient(bot_token="bot-token", room_id="<REDACTED_ROOM_ID>", require_mention=True).list_messages()
    ...
    def test_validate_env_accepts_oauth_config_with_token_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            token.parent.mkdir()
            token.write_text(
                json.dumps({"access_token": "<REDACTED_SECRET>", "refresh_token": "<REDACTED_SECRET>", "expires_at": 9999999999}),
                encoding="utf-8",
            )
            settings = Settings(
                workspace=root,
                ...
                webex_client_id="cid",
                webex_client_secret="<REDACTED_SECRET>",
                webex_room_id="room",
